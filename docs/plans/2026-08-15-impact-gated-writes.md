...
**Addition (from Task 8b):** the `confirm` param must ALSO be registered on the UpdateSource, UpdateClassInclude, ImportFromFile, DeployZip, WriteProgram, and WriteClass tools — any tool that can surface an ImpactBlockedError (all of them funnel into the primitive-gated `Client.UpdateSource` or `Client.UpdateClassInclude`), so every block response has a retry path with the token.
