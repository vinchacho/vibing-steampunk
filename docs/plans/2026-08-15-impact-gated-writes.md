...
**Addition (from Task 8b):** the `confirm` param must ALSO be registered on the UpdateSource, ImportFromFile, DeployZip, WriteProgram, and WriteClass tools — any tool that can surface an ImpactBlockedError (all of them funnel into the primitive-gated `Client.UpdateSource`), so every block response has a retry path with the token.
